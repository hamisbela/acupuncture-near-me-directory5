...
export function generateSalonPage(salon: BeautySalon): string {
  ...
  const metaDescription = `${salon.title} is an ${businessInfo.type} ${businessInfo.service} service in ${cityName}, ${stateName}. ${salon.description?.substring(0, 150) || `Contact us for ${businessInfo.serviceDetailed} services.`}`;
  ...
  if (salon.city && salon.city.salons && salon.city.salons.length > 1) {
    ...
    if (nearbySalons.length > 0) {
      ...
      <div class="mt-12 border-t border-gray-200 pt-12">
        <h2 class="text-2xl font-bold mb-6 text-indigo-800">More ${businessInfo.type.charAt(0).toUpperCase() + businessInfo.type.slice(1)} Services in ${cityName}</h2>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              <a href="${getSalonUrl(nearbySlug)}">
                ${nearbySalon.images && nearbySalon.images.length > 0 
                  ? `<img src="https://${siteInfo.cdnDomain}/${siteInfo.cdnPathPrefix}${nearbySalon.images[0].path}" alt="${nearbySalon.title}" class="w-full h-40 object-cover">`
              ...
              <div class="p-4">
                ...
                <p class="text-gray-600 mt-1 text-sm">
                  <i class="${icons.ui.location} text-indigo-500 mr-1"></i> ${cityName}, ${stateName}
                </p>
  ...
  if (salon.latitude && salon.longitude) {
    ...
    <div class="mb-6">
      ...
      >
        <i class="${icons.ui.directions} mr-2"></i> Get Directions
      </a>
    </div>
    `;
  }
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div class="relative h-40 md:h-56 bg-indigo-800">
        ${salon.images && salon.images.length > 0 
          ? `<img src="https://${siteInfo.cdnDomain}/${siteInfo.cdnPathPrefix}${salon.images[0].path}" alt="${salon.title}" class="w-full h-full object-cover opacity-70">`
          : `<div class="w-full h-full bg-gradient-to-r from-indigo-800 to-purple-800"></div>`
        }
        <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
          <div class="container mx-auto">
            <h1 class="text-2xl md:text-3xl font-bold text-white">${salon.title} - ${businessInfo.type.charAt(0).toUpperCase() + businessInfo.type.slice(1)} in ${cityName}, ${stateName}</h1>
            <div class="flex flex-wrap items-center gap-2 mt-1 text-white/90 text-sm">
              ${salon.categories && salon.categories.length > 0 ? 
                `<div class="flex items-center">
                  <i class="${icons.features.category} text-indigo-300 mr-1"></i>
                  ${salon.categories.map(cat => cat.category).join(' • ')}
                </div>` : 
                `<div class="flex items-center">
                  <i class="${icons.features.business} text-indigo-300 mr-1"></i>
                  ${businessInfo.type.charAt(0).toUpperCase() + businessInfo.type.slice(1)} Services
                </div>`
              }
              
              ${salon.city ? 
                `<div class="flex items-center ml-2">
                  <i class="${icons.ui.location} text-indigo-300 mr-1"></i>
                  ${cityName}, ${stateName}
                </div>` : ''
              }
              ...
              ${salon.telephone ? 
                `<div class="flex items-center ml-2">
                  <i class="${icons.ui.phone} text-indigo-300 mr-1"></i>
                  ${salon.telephone}
                </div>` : ''
              }
            </div>
        ...
        <div class="md:col-span-2">
          <div class="mb-8">
            <h2 class="text-2xl font-bold mb-4 text-indigo-800 flex items-center">
              <i class="${icons.features.info} text-indigo-600 mr-3"></i>
              About
            </h2>
            <div class="bg-gray-50 p-5 rounded-lg border-l-4 border-indigo-500">
              <p class="text-gray-700 leading-relaxed">
                ${salon.description || `${salon.title} provides professional ${businessInfo.type} ${businessInfo.service} services in ${cityName}, ${stateName}. Contact us to learn more about our services and to schedule an appointment.`}
              </p>
            </div>
          </div>
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-bold mb-4 text-indigo-800 flex items-center">
              <i class="${icons.features.service} text-indigo-600 mr-3"></i>
              Services
            </h2>
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-bold mb-4 text-indigo-800 flex items-center">
              <i class="${icons.features.amenity} text-indigo-600 mr-3"></i>
              Amenities
            </h2>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 bg-gray-50 p-5 rounded-lg">
              ${salon.amenities.map(amenity => 
                `<div class="flex items-center">
                  <i class="${icons.features.check} text-green-500 mr-2"></i>
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-gray-50 p-5 rounded-lg">
              ${salon.details.map(detail => 
                `<div class="flex items-center">
                  <i class="${icons.features.info} text-indigo-500 mr-2"></i>
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-bold mb-4 text-indigo-800 flex items-center">
              <i class="${icons.features.reviews} text-indigo-600 mr-3"></i>
              Reviews
            </h2>
            <div class="space-y-4">
          ...
          <div class="mb-8">
            <h2 class="text-2xl font-bold mb-4 text-indigo-800 flex items-center">
              <i class="${icons.features.gallery} text-indigo-600 mr-3"></i>
              Gallery
            </h2>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
              ${salon.images.slice(1).map(image => 
                `<a href="https://${siteInfo.cdnDomain}/${siteInfo.cdnPathPrefix}${image.path}" target="_blank" class="block h-40 overflow-hidden rounded-lg shadow-md">
                  <img src="https://${siteInfo.cdnDomain}/${siteInfo.cdnPathPrefix}${image.path}" alt="${salon.title}" class="w-full h-full object-cover hover:opacity-90 transition-opacity">
                </a>`
              ).join('')}
            </div>
          </div>
        ...
        <div>
          <div class="bg-gray-50 rounded-lg p-6 shadow-md sticky top-20">
            ...
            <div class="flex mb-4">
              <div class="text-center mr-3">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center">
                  <i class="${icons.ui.location} text-indigo-600"></i>
                </div>
              </div>
              <div>
            ...
            <div class="flex mb-4">
              <div class="text-center mr-3">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center">
                  <i class="${icons.ui.phone} text-indigo-600"></i>
                </div>
              </div>
              <div>
            ...
            <div class="flex mb-4">
              <div class="text-center mr-3">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center">
                  <i class="${icons.ui.email} text-indigo-600"></i>
                </div>
              </div>
              <div>
            ...
            <div class="flex mb-4">
              <div class="text-center mr-3">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center">
                  <i class="${icons.ui.website} text-indigo-600"></i>
                </div>
              </div>
              <div>
            ...
            <div class="mb-4">
              <h4 class="font-semibold text-gray-800 mb-3 flex items-center">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center mr-2">
                  <i class="${icons.ui.hours} text-indigo-600"></i>
                </div>
                Business Hours
              </h4>
            ...
            <div class="flex mb-5">
              <div class="text-center mr-3">
                <div class="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center">
                  <i class="${icons.ui.payment} text-indigo-600"></i>
                </div>
              </div>
              <div>
            ...
            <div class="mt-4">
              <a href="tel:${salon.telephone || ''}" 
                ...
                <i class="${icons.ui.phone} mr-2"></i> Call Now
              </a>
            </div>
          </div>
        </div>
  ...
  return pageWrapper(`${salon.title} - ${businessInfo.type.charAt(0).toUpperCase() + businessInfo.type.slice(1)} in ${cityName}, ${stateName}`, metaDescription, content);
}