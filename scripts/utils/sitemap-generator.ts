...
const BASE_URL = siteInfo.url || `https://${siteInfo.domain}`;
...
export async function generateSitemaps(
  _beautySalons: BeautySalon[], // Not used, but kept for signature compatibility
  cities: City[],
  states: State[],
  _categories: Category[] // Not used as we're skipping category pages
): Promise<void> {
...
<head>
  ...
  <title>${seoInfo.titleTemplates.sitemap}</title>
...
<body class="bg-gray-50">
  <header class="bg-indigo-900 text-white py-6">
    <div class="container mx-auto px-4">
      ...
      <p class="mt-2">A complete index of all pages on ${siteInfo.name}</p>
    </div>
  </header>
...
export async function generateRobotsTxt(multipleXmlSitemaps: boolean = false): Promise<void> {
  ...
  let robotsTxt = `# robots.txt for ${siteInfo.name}
User-agent: *
Allow: /
...
`;
  ...
  // Add HTML sitemap
  robotsTxt += `Sitemap: ${BASE_URL}${pathConfig.special.sitemap}\n`;
